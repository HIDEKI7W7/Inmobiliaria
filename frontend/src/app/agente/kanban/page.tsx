...
export default function RadarDeCierrePage() {
  ...
  // [ESTADOS_MODAL_EDITOR_MENSAJES]
  const [isMessageEditorOpen, setIsMessageEditorOpen] = useState<boolean>(false);
  const [activeStageForMessage, setActiveStageForMessage] = useState<string>('');

  const handleAbrirEditorMensaje = (stageId: string) => {
    setActiveStageForMessage(stageId);
  ...
  return (
    <div className="w-full h-full p-6 flex flex-col overflow-hidden bg-[#F8FAFC]">
      ...
      <div className="flex-1 w-full overflow-x-auto overflow-y-hidden flex gap-6 pb-4 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
        ...
        {STAGES.map((stage) => {
          ...
          return (
            ...
            >
              {/* [JSX_REDISEÑO_CABECERA_COLUMNA_KANBAN] */}
              <div className={`flex justify-between items-start mb-4 flex-shrink-0 border-b pb-3 border-slate-200/50`}>
                {/* Lado izquierdo: etiqueta + subetiqueta */}
                ...
                {/* Lado derecho: contador + botón editor de mensaje */}
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    title={`Editar plantilla de mensaje para etapa: ${stage.label}`}
                    onClick={() => handleAbrirEditorMensaje(stage.id)}
                    className="flex items-center gap-1.5 p-1.5 rounded-lg bg-gray-50 border border-gray-200 text-gray-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm cursor-pointer active:scale-95"
                  >
                    {/* Icono de mensaje/plantilla */}
                    ...
                    <span className="text-[11px] font-semibold leading-none">Mensaje</span>
                  </button>
                </div>
              </div>
      ...
      {/* [JSX_MODAL_EDITOR_MENSAJES] — Editor de Plantilla de Mensaje por Etapa */}
      {isMessageEditorOpen && (
        <div
          className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fadeIn"
          onClick={() => setIsMessageEditorOpen(false)}
          role="dialog"
          aria-modal="true"
          aria-label="Editor de plantilla de mensaje"
        >
          <div
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-blue-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
                  </svg>
                </div>
                <div>
                  <p className="text-xs font-black text-[#04045E] uppercase tracking-wider">Plantilla de Mensaje</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                    Etapa: <span className="text-blue-600">{STAGES.find(s => s.id === activeStageForMessage)?.label || activeStageForMessage}</span>
                  </p>
                </div>
              </div>
              <button
                onClick={() => setIsMessageEditorOpen(false)}
                className="text-slate-400 hover:text-slate-700 font-bold p-1.5 hover:bg-slate-100 rounded-full transition-all cursor-pointer"
            ...
            <div className="px-5 py-4 space-y-3">
              <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">
                Mensaje automatizado para WhatsApp / Notificación
              </label>
              <textarea
                id="textarea-plantilla-mensaje"
                rows={5}
                defaultValue={
                  activeStageForMessage === 'NUEVO'
                    ? `Hola {{nombre}}, te saluda tu Asesor Comercial de Propio. Vi que te interesa la propiedad "{{interes}}". Me pongo a tu servicio por este canal privado.`
                    : activeStageForMessage === 'CONTACTADO'
                    ? `Hola {{nombre}}, siguiendo nuestro contacto anterior, ¿tienes disponibilidad esta semana para coordinar una visita al inmueble que te interesa?`
                    : activeStageForMessage === 'VISITA'
                    ? `Hola {{nombre}}, te confirmo tu visita agendada para el {{fecha}} a las {{hora}}. Estaré esperándote. ¡Nos vemos pronto!`
                    : `Hola {{nombre}}, estamos muy cerca de cerrar el trato. Te envío la propuesta final para tu revisión. ¿Alguna duda?`
                }
                className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-medium text-slate-800 focus:outline-none focus:border-blue-400 transition-colors bg-slate-50 resize-none"
              />
              <p className="text-[9px] text-slate-400 font-bold">
                Variables disponibles: <span className="text-blue-500">{'{{nombre}}'}</span> · <span className="text-blue-500">{'{{interes}}'}</span> · <span className="text-blue-500">{'{{fecha}}'}</span> · <span className="text-blue-500">{'{{hora}}'}</span>
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-slate-100 bg-slate-50">
              <button
                type="button"
                onClick={() => setIsMessageEditorOpen(false)}
                className="bg-gray-100 text-gray-700 rounded-xl px-5 py-2 text-xs font-black uppercase tracking-wider hover:bg-gray-200 transition-all cursor-pointer active:scale-95"
              >
                Cancelar
              </button>
              <button
                type="button"
                id="btn-guardar-plantilla-mensaje"
                onClick={() => setIsMessageEditorOpen(false)}
                className="bg-[#04045E] hover:bg-[#04045E]/90 text-white rounded-xl px-5 py-2 text-xs font-black uppercase tracking-wider transition-all cursor-pointer active:scale-95 shadow-sm flex items-center gap-2"
              >
